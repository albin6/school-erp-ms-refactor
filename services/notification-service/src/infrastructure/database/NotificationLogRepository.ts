...
export class NotificationLogRepository {
    ...
    async claimEventProcessing(
        ...
        const { rowCount } = await getPool().query(
           ...
           event_id, event_type, recipient, channel, status, error_message, updated_at, heartbeat_at
         ...
         WHERE notification_logs.status = 'FAILED'
            OR (
                 ...
                 AND (
                      notification_logs.heartbeat_at IS NULL
                      OR notification_logs.heartbeat_at < NOW() - ($5::text || ' minutes')::interval
                 )
               )`,
    ...
    async touchHeartbeat(eventId: string, recipient: string, channel: string): Promise<void> {
       ...
       WHERE event_id = $1 AND recipient = $2 AND channel = $3 AND status = 'PROCESSING'`,
    ...
    async markEventProcessed(
        ...
        await getPool().query(
            `UPDATE notification_logs
         SET status = $4, error_message = $5, updated_at = NOW(), heartbeat_at = NULL
