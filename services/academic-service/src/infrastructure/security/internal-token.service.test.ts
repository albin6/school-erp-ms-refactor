...
describe('verifyInternalAuthToken', () => {
    it('accepts a valid gateway-issued academic-service token', () => {
        const payload = verifyInternalAuthToken(createToken());
    ...
    it('rejects a tenant-service scoped token', () => {
        expect(() => verifyInternalAuthToken(createToken({ aud: 'tenant-service' })))
