    ...
    try {
        ...
        for (const row of rows) {
            try {
                ...
                await publishEvent(row.event_type, row.aggregate_id, payload);
                await pool.query(
                    `UPDATE outbox_events
                ...
                logger.error(`Outbox worker failed to process event ${row.id}`, { error: error.stack });
            }
        }
    } catch (error: any) {
...
export const startOutboxWorker = () => {
    ...
    logger.info(' Academic outbox worker started');
    void processOutbox();
};
...
export const stopOutboxWorker = () => {
    ...
    logger.info('Academic outbox worker stopped');
};
