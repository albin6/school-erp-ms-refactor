...
const scheduleReconnect = (reason: string, error?: string) => {
    ...
    logger.warn(`Kafka producer unavailable; academic event publishing is degraded. Retrying in ${delayMs}ms`, {
        attempt: nextAttempt,
        reason,
        error,
    });
...
export const connectProducer = async (reason = 'startup'): Promise<void> => {
    ...
    connectPromise = (async () => {
        try {
            ...
            logger.warn('Kafka producer connection attempt failed; academic event publishing remains degraded', {
                reason,
                error: errorMessage,
            });
            try {
