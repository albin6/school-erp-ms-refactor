...
export const TenantStaffLogin = () => {
    const { slug } = useParams<{ slug: string }>();
    const [loading, setLoading] = useState(false);
    const [pageLoading, setPageLoading] = useState(false);
    const [branchName, setBranchName] = useState<string>('');
    const navigate = useNavigate();
    const { loginStaff } = useTenantAuthStore();
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 to-cyan-100">
            <Card className="w-full max-w-md shadow-2xl">
                <div className="text-center mb-8">
                    <Title level={2} className="mb-2">Staff Portal</Title>
                ...
                <div className="text-center mt-4">
                    <Text type="secondary">
                        <Link to="/admin" className="text-blue-600 hover:text-blue-700 mx-2">Admin Login</Link>
                        |
                        <Link to={`/${slug}`} className="text-blue-600 hover:text-blue-700 mx-2">Student Login</Link>
                    </Text>
                </div>
            </Card>
        </div>
